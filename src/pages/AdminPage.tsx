...
const TESTDATEN_FIELDS = [
  ...
  { key: 'kategorie', label: 'Kategorie', type: 'lookup/select', options: [{ key: 'option_b', label: 'Option B' }, { key: 'option_c', label: 'Option C' }, { key: 'option_a', label: 'Option A' }] },
  { key: 'bemerkungen', label: 'Bemerkungen', type: 'string/textarea' },
];

const ENTITY_TABS = [
  { key: 'testdaten', label: 'Testdaten', pascal: 'Testdaten' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('testdaten');
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'testdaten': return (data as any).testdaten as Testdaten[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'testdaten': return TESTDATEN_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'testdaten': return {
        create: (fields: any) => LivingAppsService.createTestdatenEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateTestdatenEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteTestdatenEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'testdaten' || dialogState?.entity === 'testdaten') && (
        <TestdatenDialog
          open={createEntity === 'testdaten' || dialogState?.entity === 'testdaten'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'testdaten' ? handleUpdate : (fields: any) => handleCreate('testdaten', fields)}
          defaultValues={dialogState?.entity === 'testdaten' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['Testdaten']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Testdaten']}
        />
      )}
      {viewState?.entity === 'testdaten' && (
        <TestdatenViewDialog
          open={viewState?.entity === 'testdaten'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'testdaten', record: r }); }}
        />
      )}
